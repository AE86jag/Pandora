CREATE TABLE IF NOT EXISTS `fund_fixed_investment_condition` (
    ...
    `first_category_of_cycle`   VARCHAR(10) NOT NULL COMMENT '周期大类',
    `second_category_of_cycle`  VARCHAR(10) NOT NULL COMMENT '周期子类',
    ...
    `created_time`              TIMESTAMP COMMENT '最后修改时间',
    `last_modified_time`        TIMESTAMP COMMENT '创建时间',
    PRIMARY KEY(`id`)
